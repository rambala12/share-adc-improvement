...
//***********************************************EASY**FIND****************************************************\\
// Check results queue for information
// Add any needed requests to requests queue

/* Read touch screen. Returns 1 if touched, and updates position. Else returns 0 leaving 
position unchanged. */
uint32_t LCD_TS_Read(PT_T * position) {
	uint32_t x, y; // coordinates of where touch happens
	uint32_t b;
	
	osStatus_t q_status; // observe status of queue
	RESULT_VALUES dest_msg; // use to get value from results queue
	static REQUEST_VALUES msg1; // use to put message into requests queue
	static REQUEST_VALUES msg2;
...
#if USE_ADC_FOR_TOUCHSCREEN
	// Read inputs
	// add to request queue for x coordinate
		msg1.channelNum = LCD_TS_YU_CHANNEL;
		osMessageQueuePut(requests_q, &msg1, NULL, 0U);
		osMessageQueueGet(results_q, &dest_msg, NULL, osWaitForever);
		yu = dest_msg.result;
#endif
		x = yu; // grab value from results queue
...
#if USE_ADC_FOR_TOUCHSCREEN
		// Read inputs
		// Add to request queue for y coordinate
		msg2.channelNum		= LCD_TS_XL_CHANNEL;
		osMessageQueuePut(requests_q, &msg2, 0U, 0U);
		osMessageQueueGet(results_q, &dest_msg, NULL, osWaitForever);
		xl = dest_msg.result;
#endif
		y = xl; // grab value from results queue
		
		//UI_Process_Touch uses x and y values to update HBLED values
